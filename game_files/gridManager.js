...
function getNextCase(grid, kindMove, caseType, lastCase) {
  ...
  if (lastCase) {
    ...
    if (
      kindMove == enumCaseParser.InARow ||
      kindMove == enumCaseParser.Horizontal
    )
      iterator++;
    else if (kindMove == enumCaseParser.Vertical) iterator += grid.nbLines;
  }

  // Don't bypass array length !
  if (iterator >= grid.cases.length) return null;
...
function insertDescription(grid, desc) {
  var currentCase = getNextCase(
      grid,
      enumCaseParser.InARow,
      enums.CaseType.Description
    ),
    assigned = false;
...
function getCaseType(Char) {
  if (Char == "z") return enums.CaseType.Empty;
  else if (Char >= "A" && Char <= "Z") return enums.CaseType.Letter;
  else return enums.CaseType.Description;
}
  ...
  for (i = 0; i < length; i++) {
    ...
    if (info[0].indexOf("lign") > -1) {
      for (j in info[1]) {
        ...
        else grid.cases.push(new Case.EmptyCase(currentCase++));
      }

      // Update col & line counters
      if (grid.nbLines == 0) grid.nbLines = info[1].length;
      grid.nbColumns++;
    }
    ...
    // If this line set a dotted frame, apply the effect to the right frame
      ...
      info[0].indexOf("pointillev") > -1 ||
      info[0].indexOf("pointilleh") > -1
    ) {
  ...
  for (i = 0; i < gridSize; i++) {
    if (grid.cases[i].type == enums.CaseType.Description) {
      ...
      switch (grid.cases[i].value) {
        ...
        default:
          console.error(
            "[ERROR][gridManager::placeArrows] Unknow arrow type [" +
              grid.cases[i].value +
              "] at frame " +
              i
          );
...
function getGridAddress(commandArgv) {
  var gridNumber, today, gridDefaultDay, dayDiff;
  ...
  return (
    config.PROVIDER_ADDR + gridNumber.toString() + config.PROVIDER_EXTENSION
  );
}
...
GridManager.prototype.checkPlayerWord = function(wordObj) {
  ...
  for (i = 0; i < wordSize; i++) {
    ...
    if (wordObj.word[i] != _grid.cases[index].value) return -1;

    if (_grid.cases[index].available == true) points++;
 ...
 * @return {Int}    The number of words still available
 */
GridManager.prototype.getAccomplishmentRate = function(
  playerPoints,
  nbPlayers
) {
...
GridManager.prototype.retreiveAndParseGrid = function(gridNumber, callback) {
  var gridAddr = getGridAddress(gridNumber), // Retreive the grid URL, build from provider infos and ID requested
    req = https.get(gridAddr, function(res) {
      // Launch the request !
      ...
      if (res.statusCode !== 200 && res.statusCode !== 302) {
        ...
        res
          ...
          .on("end", function() {
            ...
            console.info(
              "\n\t[GRIDMANAGER] Parsing Done. Now play " +
                _gridInfos.provider +
                " " +
                _gridInfos.id +
                " - Level " +
